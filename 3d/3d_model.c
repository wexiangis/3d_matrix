 ...
 *  参数:
 *      pCount: 点个数
 *      x, y, z, color: 第一个点的坐标和颜色参数, 其中x,y,z必须写成0.00的格式, 如3写成3.00
...
_3D_Model *_3d_model_init(uint32_t pCount, double x, double y, double z, uint32_t color, ...)
{
    ...
    model->pCount = pCount;
    model->pArray = (_3D_Point *)calloc(pCount, sizeof(_3D_Point));
    //记录第一个点
    model->pArray[count].x = x;
    model->pArray[count].y = y;
    model->pArray[count].z = z;
    model->pArray[count].color = color;
    count += 1;
    //记录其它点
    if (count < pCount)
    {
        va_start(ap, color);
        for (; count < pCount; count++)
        {
            model->pArray[count].x = va_arg(ap, double);
            model->pArray[count].y = va_arg(ap, double);
            model->pArray[count].z = va_arg(ap, double);
            model->pArray[count].color = va_arg(ap, uint32_t);
        }
        va_end(ap);
    }
    return model;
...
_3D_Model *_3d_model_copy(_3D_Model *model)
{
    ...
    model2->pArray = (_3D_Point *)calloc(model2->pCount, sizeof(_3D_Point));
    memcpy(model2->pArray, model->pArray, sizeof(_3D_Point) * model2->pCount);
    //net链表拷贝
    if (model->net)
    {
        net = model->net;
